...
const PROBES = [
  ...
  ['th.c-name',                  'toggle',   () => document.querySelector('th.c-name')],
  ...
  // Deliberately constant: text on a saturated accent fill stays light in
  // both themes by design, so a frozen result here is correct.
...
def main():
    ...
    for name in expect:
        ...
        if l is None or d is None:
            missing.append(name)
            continue
        differing = [p for p in l if l[p] != d[p]]
        if differing:
    ...
    if frozen_defects:
        ...
        for name, _, l, _ in frozen_defects:
            shown = {p: v for p, v in l.items() if v and v != "rgba(0, 0, 0, 0)"}
    ...
    return 1 if frozen_defects else 0
