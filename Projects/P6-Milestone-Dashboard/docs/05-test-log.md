...
| TEST-50 | 2026-09-23 | D-15a Top filter bar: grouped, sized and themed correctly at phone, tablet and desktop widths, on touch and fine pointers | `tools/d15a_check.py` (new) at 390, 768 and 1440: no horizontal overflow, equal field heights, clear buttons inside their fields, even gaps between groups, the bar not clipped, icon clear of placeholder, 44px and 16px on touch, full-width fields on a phone, borderless close, labels next to controls on desktop. `theme_check.py` gains filter label and select probes, shown failing on the P46 build before passing here. `d15_check.py` made deterministic | **d15a 28/28; d15 11/11 on 5 of 5 runs; theme_check 0 frozen; p28 34/34, p29 53/53, p34 73/73, p35 120/120, p39 117/117, p43 36/36, p44 31/31, p46 37/37.** Hardcoded colour occurrences unchanged against P45 | TD-184 |
| TEST-49 | 2026-09-23 | D-15 control consistency pass: dialog shadows resolve, every icon-only button is labelled, one keyboard focus ring and one minimum hit target, the relabelled reset button, and the card's text save pair | `tools/d15_check.py` (new) measures computed `box-shadow` on `#ms-dialog` in both themes, walks every button for an accessible name, forces keyboard focus and reads the outline, and asserts the relabel. Placement probes p32, p34 and p35 were re-run because the hit-target rule reaches controls inside board rows | **d15 11/11; theme_check 0 frozen; p29 53/53, p32 36/36, p34 73/73, p35 120/120, p39 117/117, p43 36/36, p44 31/31, p46 37/37.** Hardcoded colour occurrences unchanged against P45. Two existing assertions updated for the intentional relabel (p29, p39) | TD-182, TD-183 |
