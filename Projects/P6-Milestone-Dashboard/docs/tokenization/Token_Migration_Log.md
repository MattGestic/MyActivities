...
| 2026-09-26 | Audit script v3, after v3.1.0-P54 (D-21 histogram component: `tr.hist-row td.c-name`'s header-surface pairing and italic replaced with the panel small-caption pair; the numeric side cells moved off `--color-bg-header`/`--color-text-on-header-muted` onto `--color-bg-subtle`/`--color-text-on-panel-muted`; new measure/position segmented controls built entirely from existing `--ctl-*`/`--color-vt-divider`/`--color-accent-purple` tokens, no new colour values) | Hardcoded colour occurrences | 38 (unchanged) |
| 2026-09-26 | Audit script v3, after v3.1.0-P54 | Distinct hardcoded colour values | 34 (unchanged) |
| 2026-09-26 | Audit script v3, after v3.1.0-P54 | `--space-*` token references in file | 132 |
| 2026-09-26 | Audit script v3, after v3.1.0-P54 | Hardcoded padding/margin/gap declarations (px) | 227 (unchanged — new gaps/padding tokened onto `--space-1/2/3`) |
| 2026-09-26 | Audit script v3, after v3.1.0-P54 | `var(--text-*)` font-size references in file | 41 (unchanged) |
...
| Histogram row + label cell (D-21) | Tokenized — the label cell was using the HEADER pairing (`--color-bg-header`/`--color-text-on-header-muted`) plus italic on a PANEL-context row, which is the same class of latent trap TD-46 named: a token used outside the surface it was paired against. Now `--color-bg-subtle`/`--color-text-small` (c-name) and `--color-bg-subtle`/`--color-text-on-panel-muted` (the numeric side cells), non-italic. The new measure/position segmented controls reuse the `.view-toggle`/`.vt-btn` token pair (`--color-vt-divider`, `--color-accent-purple`, `--color-text-on-accent`) rather than a third accent look | 2026-09-26 |
