...
| TASK-50 | FEAT-05, FEAT-07 | Milestone card rework: layout, and an editable Progress field | Open | Requested 2026-09-17 with the P32 defects, deferred here rather than bundled into a defect fix. Five parts. (1) Shrink the dialog about 10%. (2) Start, Finish and Progress on one row, with a column divider between Finish and Progress, and Start and Finish at the same text size; today Start is a green actual-date chip and Finish is larger plain text, and Progress sits a row below. (3) Beneath that, a collapsible row holding MS weight, MS hours and earned hours, which are reference values rather than things read at a glance. (4) Progress becomes editable, carrying the same `.changed` edited indicator the comment field uses, so an overridden value is visibly an override; clearing it and leaving the field restores the schedule's own value rather than saving a blank. (5) Marking the icon complete sets Progress to 100%. Note for whoever builds it: a Progress override is annotation-layer state, so it keys through `msKeyFor()` alongside `MS_COMMENTS` and `MS_HEALTH_OVERRIDE`, needs its own `ANNOT_CATEGORIES` entry, and must round-trip through publish and the model export. It must never write back into the milestone record, per the three-layer rule. |
