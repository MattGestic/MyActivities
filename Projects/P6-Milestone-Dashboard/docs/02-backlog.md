...
| TASK-50 | FEAT-05, FEAT-07 | Milestone card rework: layout, and an editable Progress field | Done | Requested 2026-09-17 with the P32 defects, deferred here rather than bundled into a defect fix. Five parts. (1) Shrink the dialog about 10%. (2) Start, Finish and Progress on one row, with a column divider between Finish and Progress, and Start and Finish at the same text size; today Start is a green actual-date chip and Finish is larger plain text, and Progress sits a row below. (3) Beneath that, a collapsible row holding MS weight, MS hours and earned hours, which are reference values rather than things read at a glance. (4) Progress becomes editable, carrying the same `.changed` edited indicator the comment field uses, so an overridden value is visibly an override; clearing it and leaving the field restores the schedule's own value rather than saving a blank. (5) Marking the icon complete sets Progress to 100%. Note for whoever builds it: a Progress override is annotation-layer state, so it keys through `msKeyFor()` alongside `MS_COMMENTS` and `MS_HEALTH_OVERRIDE`, needs its own `ANNOT_CATEGORIES` entry, and must round-trip through publish and the model export. It must never write back into the milestone record, per the three-layer rule. **Delivered v3.1.0-P35, all five parts.** The card is 306px against 340 (10.0% narrower, 22.6% shorter, measured against the P34 release in the same browser). `MS_PROGRESS_OVERRIDE` is the annotation store; `effectiveProgress()` is the only reader and every consumer goes through it, including `computeProgress()`, so the row rollup and the tooltip move with the card. An override equal to the schedule's own value is discarded rather than stored, which is the opposite of the health store and is recorded as a decision. Marking complete sets 100% in one direction only. TEST-37, 117/117. |
| TASK-51 | FEAT-07 | More Actions: collapse the seven header icon buttons into one persistent menu, with the app name and version text beside it | Done | Requested with TASK-50 and deferred behind it. Had no backlog entry until it was built, existing only in the to-do's next-task line, which is why this row is written at closure. v3.1.0-P36, TD-134. The rows keep their ids so no state-writing function changed. Measured gain: the version label goes from 35.2px to 215.8px of the 216px it needs at 768 wide, and 228px of header width is returned. TD-135 records the residual clip at 390px. |
| TASK-52 | FEAT-05, FEAT-07 | Fix the four defects reported against the P36 build | Done | v3.1.0-P37, TD-137 to TD-140, TD-141 raised and left open. Two of the four reports were not what they looked like: the Title col slider was refuted by the first measurement and confirmed by a wider one, and the zero-dependency filter was a label/behaviour mismatch rather than a rendering fault. TEST-39, 65/65. |
| TASK-53 | FEAT-07, FEAT-13 | Print preview: align the heading bars with the sheet, and give the preview its own controls | Done | Requested 2026-09-22 against the P37 build with two screenshots. v3.1.0-P38, TD-142. `#icon-bar` and `.pm-banner` took the viewport's width against a 1122.5px A3 sheet, measuring 390px at phone width and 1440px at desktop, so they were short of the sheet in one direction and over it in the other; both now take the sheet's width, centring and side inset, and the content boxes coincide with the report heading as well as the outer edges. The banner carries three controls: re-fit to page width, leave the preview, and the same More Actions panel. The panel is NOT duplicated: one panel, and `moreActionsAnchor()` resolves which trigger it hangs off, so the seven row ids and the five functions that write them are untouched. TEST-40. |
| TASK-54 | FEAT-05, FEAT-06 | Date range applies on set; default view opens with Activity ID labels and no hours labels | Done | Requested with TASK-53. v3.1.0-P38, TD-143 to TD-145. The date range already supported one open end and the first measurement refuted the report; the defect was `change` on a date input not firing until the field is left, fixed with `oninput` through the existing debounce. The default change surfaced TD-145, a first-paint path that hid only one of the three board labels, now folded into the single `reapplyDisplaySettings()` writer. TEST-40, 91/91. |
| TASK-55 | FEAT-05, FEAT-06, FEAT-07 | View Controls rework, Baseline shadow in the heading, the drawer's action bar, and a two-column filter row | Done | Eight items requested together on 2026-09-22 with four screenshots. v3.1.0-P39, TD-146 to TD-150, TD-151 and TD-152 raised. Layout section reordered and renamed (Title Column, Column Width, Row height, Icon size) with Column Width reaching 144px and the fit clamp raised to match, so fitting cannot beat dragging. Each label size slider sits under its own control and is dead while that text is off. Remarks became `Row Comments field` with a Show / Hide pair. The baseline overlay toggle moved beside the View toggle and its note became its tooltip. The drawer's actions moved above the tabs, right anchored, with the destructive action on its own line. The filter row is two wrapping columns with a rule before the date range. The screenshots came from a pre-P36 build, so the duplicate "Title contains" field they asked to remove was already gone and the dependency report is refuted (TD-151). TEST-41, 108/108. |
| TASK-56 | FEAT-05, FEAT-06, FEAT-07, FEAT-09, FEAT-13 | Dense-row growth, the critical-path filter set, the split CSV, and adding a milestone | Done | Six items requested together on 2026-09-22 with two screenshots. v3.1.0-P40, TD-153 to TD-157, TD-158 raised. A row whose densest proximity run reaches the band count grows by half again. The filter bar became three rows, one per question, with a new critical-path container holding a multi-select status chip group and a float filter in days or weeks. The drawer's action bar went back to one row at the user's request, reversing the P39 arrangement. The CSV split into a schedule block and an entered block, asserted on content rather than headers. Milestones can be added from a header button or a double click on a cell, as annotation-layer state that round-trips and never touches the seed arrays. Two things left open with the user: row 51 is identically dense to rows 37 and 46 on the unfiltered board and is grown, contrary to the report, which the measurement explains; and no recorded convention for user-added Activity IDs was found, so USR-NNN was chosen. TEST-42, 77/77. |
| TASK-58 | FEAT-05, FEAT-07 | One status vocabulary, and black for complete on upload against green for done in this update | Done | v3.1.0-P42, TD-160 closed, TD-161 raised. The colours were already right; `effectiveState()` was folding a person's override onto the schedule's own DONE, so a new DONEUSER state takes it. Internal key stays DONE because the seed data carries it; health numbers 0 to 3 keep their numbers so published files do not re-colour; FUTURE kept as a real schedule state; Complete is not settable by a person, black being reserved for what the upload records. TEST-44, 25/25. |
| TASK-57 | FEAT-05, FEAT-06 | Row growth follows the markers on screen, not the row's total | Done | The alternative TD-153 left open, chosen by the user on 2026-09-23. v3.1.0-P41, TD-153 closed, TD-159 raised and closed. The four reported rows now behave exactly as reported under the report's own date range: 18 and 51 plain, 37 and 46 grown. Row height stays render-time state, so the filter pass owes a rebuild rather than resizing in place, and that check has to sit at all three entry points that change which columns are on the board. The third, clearFilter(), was missed on the first pass and caught by an existing probe. TEST-43, 91/91. |
