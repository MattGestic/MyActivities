...
| Design token block (colour/spacing/text) | P6 Milestone Dashboard `:root{}` and theme blocks | A worked light/dark token set with a documented consolidation methodology. | R1 and R2 — colour values are project-specific; the *methodology* may be the reusable part, not the values. Also blocked on the missing Token Migration Log. |
